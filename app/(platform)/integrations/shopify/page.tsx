...
import {
  useShopifyStatus,
  useShopifyWebhookEvents,
  useShopifyConversions,
  startShopifyConnect,
  disconnectShopify,
  triggerShopifySync,
...
// Shopify brand green. Used where Salla/Zid use the theme primary for
// the disconnected hero icon, so the page reads on-brand at a glance.
const SHOPIFY_GREEN = "#96BF48";

/**
 * /integrations/shopify — the Shopify integration management page.
 *
 * Three states:
 *   1. Feature disabled at platform level → static info card
 *   2. Not connected → "Connect your store" CTA (asks for the shop
 *      domain, then fires OAuth for that specific store)
 *   3. Connected → status panel + tabs for settings, conversions, etc.
 *
 * The OAuth callback lands on /integrations/shopify?shopify_status=connected
 * (or =error) — we read those params on mount and surface a toast.
 *
 * Mirrors /integrations/zid structure deliberately so operators
 * familiar with one can navigate the other. Shopify-specific
 * divergences: a shop_domain input before connecting, permanent tokens
 * (no expiry row in Overview), and Settings without a cart-recovery
 * block.
 */
export default function ShopifyIntegrationPage() {
  return (
    <Suspense fallback={null}>
      <ShopifyIntegrationInner />
    </Suspense>
  );
}

function ShopifyIntegrationInner() {
  ...
  const { data, isLoading, mutate } = useShopifyStatus(15000);
  ...
  useEffect(() => {
    ...
    if (status === "connected") {
      ...
      showToast(
        ar
          ? `تمّ ربط متجر شوبيفاي${store ? ` (${store})` : ""} بنجاح ✅`
          : `Shopify store${store ? ` (${store})` : ""} connected ✅`,
      );
    } else if (status === "error") {
      const msg = searchParams.get("shopify_message");
      showToast(
  ...
  const handleConnect = async (shopDomain: string, onError: (msg: string) => void) => {
    setBusy("connect");
    try {
      const url = await startShopifyConnect(shopDomain);
      if (url) {
      ...
      // Surface backend 422 validation messages inline next to the
      // input; fall back to a toast for unexpected failures.
      const data422 = e?.response?.data;
      const fieldMsg = data422?.errors?.shop_domain?.[0];
      const msg = fieldMsg || data422?.message || e?.message || "Unable to start connection";
      onError(msg);
      setBusy("");
    }
  };
  ...
  const doDisconnect = async () => {
    ...
    try {
      ...
      showToast(ar ? "تمّ فصل شوبيفاي" : "Shopify disconnected");
      await mutate();
    } catch (e: any) {
      ...
      <div style={{ padding: "0 24px 24px", maxWidth: 720, margin: "0 auto" }}>
        ...
        <Card style={{ padding: 28, marginTop: 16, textAlign: "center" }}>
          ...
          <h2 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 700, color: C.txt }}>
            {ar ? "تكامل شوبيفاي قيد التحضير" : "Shopify integration is being prepared"}
          </h2>
  ...
  // A tenant who downgraded to Basic with a live integration keeps the
  // management UI (and the disconnect button) intact; the gate only
    ...
    return (
      <>
        ...
        <Modal
          ...
          title={ar ? "تأكيد فصل شوبيفاي" : "Disconnect Shopify"}
  ...
  // their plan doesn't include Shopify. Swaps in for the Connect CTA.
  ...
  return (
    <div style={{ padding: "0 24px 24px", maxWidth: 720, margin: "0 auto" }}>
      ...
      <Card style={{
        ...
        <div style={{ fontSize: 36, marginBottom: 12 }}>🛍️</div>
        <h2 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 700, color: C.txt }}>
          {ar ? "تكامل شوبيفاي — متاح من Starter وأعلى" : "Shopify — available on Starter and above"}
        </h2>
        <p style={{ margin: "0 0 18px", fontSize: 13, color: C.t2, lineHeight: 1.7 }}>
          {ar
            ? "لتمكين الربط مع متجر شوبيفاي، استقبال الطلبات تلقائياً، وإرسال رسائل تأكيد للعملاء، رقّ باقتك."
            : "Connect a Shopify store, receive orders automatically, and send confirmation messages to customers by upgrading your plan."}
        </p>
        <a
          href="/billing"
          style={{
...
function NotConnectedPanel({
  ...
  ar: boolean; C: any; busy: string;
  onConnect: (shopDomain: string, onError: (msg: string) => void) => void;
}) {
  // Shopify is per-shop: the merchant must tell us which store to
  // connect before we can build an OAuth URL. We validate loosely
  // (non-empty) client-side; the backend sanitises + fully validates.
  ...
  const submit = () => {
    ...
    if (!trimmed) {
      setInputError(ar ? "أدخل رابط متجرك على شوبيفاي" : "Enter your Shopify store domain");
      return;
    }
    setInputError(null);
    onConnect(trimmed, (msg) => setInputError(msg));
  };

  const benefits = ar
    ? [
        { icon: "chart", t: "تتبّع التحويلات الحقيقي", d: "ROI دقيق لكلّ حملة بدلاً من تقديري" },
        { icon: "bell",  t: "إشعارات الطلبات تلقائياً", d: "تأكيد وشحن — كلّها على واتساب" },
        { icon: "users", t: "مزامنة العملاء", d: "كلّ عملائك في شوبيفاي يدخلون Corbit تلقائياً" },
        { icon: "refresh", t: "مزامنة تلقائيّة", d: "الطلبات تُسحَب باستمرار بلا تدخّل يدوي" },
      ]
    : [
        { icon: "chart", t: "Real conversion tracking", d: "Accurate ROI per campaign, not estimated" },
        { icon: "bell",  t: "Auto order notifications", d: "Confirmation + shipping — on WhatsApp" },
        { icon: "users", t: "Customer sync", d: "Every Shopify customer auto-imported to Corbit" },
        { icon: "refresh", t: "Automatic sync", d: "Orders pulled continuously, no manual work" },
      ];
    ...
    <div style={{ padding: "0 24px 24px", maxWidth: 720, margin: "0 auto" }}>
      ...
      <Card style={{ padding: 28, marginTop: 16 }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ marginBottom: 12, display: "flex", justifyContent: "center", color: SHOPIFY_GREEN }}><BrandIcon name="shopify" size={48} /></div>
          <h1 style={{ margin: "0 0 8px", fontSize: 22, fontWeight: 800, color: C.txt }}>
            {ar ? "اربط متجرك على شوبيفاي" : "Connect your Shopify store"}
          </h1>
          <p style={{ margin: 0, fontSize: 13, color: C.t2, lineHeight: 1.7 }}>
            {ar
              ? "اربط متجرك على منصّة شوبيفاي لتفعيل تتبّع التحويلات وإشعارات الطلبات الذكيّة."
              : "Connect your Shopify store to enable conversion tracking and smart order notifications."}
          </p>
        </div>
        ...
        {/* Shop domain input — required before we can fire OAuth. */}
        <div style={{ maxWidth: 420, margin: "0 auto 4px" }}>
          <label style={{ display: "block", fontSize: 12.5, color: C.t2, marginBottom: 6, fontWeight: 600 }}>
            {ar ? "رابط متجرك على شوبيفاي" : "Your Shopify store domain"}
          </label>
          <input
            type="text"
            dir="ltr"
            value={shopDomain}
            onChange={(e) => { setShopDomain(e.target.value); if (inputError) setInputError(null); }}
            onKeyDown={(e) => { if (e.key === "Enter" && busy !== "connect") submit(); }}
            placeholder="mystore.myshopify.com"
            style={{
              width: "100%", padding: "10px 12px", borderRadius: 8,
              border: `1px solid ${inputError ? C.danger : C.brd}`,
          ...
          <div style={{ fontSize: 11, color: C.t3, marginTop: 6 }}>
            {ar
              ? "مثال: mystore.myshopify.com (تجده في إعدادات متجرك)"
              : "Example: mystore.myshopify.com (found in your store settings)"}
          </div>
        </div>

        <div style={{ textAlign: "center", marginTop: 18 }}>
          <Button primary onClick={submit} disabled={busy === "connect"}>
            {busy === "connect"
              ? (ar ? "جارٍ التحويل لشوبيفاي..." : "Redirecting to Shopify...")
              : (ar ? "ربط متجر شوبيفاي الآن" : "Connect Shopify store now")}
          </Button>
          <p style={{ margin: "12px 0 0", fontSize: 11, color: C.t3 }}>
            {ar
              ? "ستُحوَّل لصفحة شوبيفاي لتأكيد الربط، ثمّ تعود لهنا تلقائياً."
              : "You'll be redirected to Shopify to confirm, then back here automatically."}
          </p>
        </div>
      </Card>
...
function ConnectedPanel({
  ...
  // Shopify has no 'expired' state — tokens are permanent.
  const statusColor = integration.status === "active" ? C.ok :
    integration.status === "error" ? C.danger : C.warn;
  const statusLabel = ar
    ? ({ active: "نشط", disconnected: "مفصول", error: "خطأ" } as any)[integration.status] ?? integration.status
    : integration.status;
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div
            style={{
              ...
              background: `${SHOPIFY_GREEN}18`,
              display: "flex", alignItems: "center", justifyContent: "center",
              color: SHOPIFY_GREEN,
            }}
          >
            <BrandIcon name="shopify" size={30} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: C.txt }}>
                {integration.store_name || (ar ? "متجر شوبيفاي" : "Shopify Store")}
              </h2>
            ...
            <div style={{ fontSize: 12, color: C.t2 }}>
              {integration.store_url ? (
                ...
                <span style={{ direction: "ltr", unicodeBidi: "plaintext" }}>{integration.shop_domain}</span>
              ) : null}
...
function OverviewTab({ integration, ar, C }: { integration: any; ar: boolean; C: any }) {
  ...
  // No token expiry row — Shopify access tokens are permanent.
  const rows = [
    { lblAr: "رابط المتجر", lblEn: "Shop domain", val: integration.shop_domain, ltr: true },
    { lblAr: "اسم المتجر", lblEn: "Store name", val: integration.store_name },
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={{ borderBottom: i < rows.length - 1 ? `1px solid ${C.brd}` : "none" }}>
              ...
              <td style={{ padding: "12px 8px", color: C.txt, ...(r.ltr ? { direction: "ltr" as const, unicodeBidi: "plaintext" as const } : {}) }}>
                {r.val || "—"}
              </td>
            </tr>
          ))}
...
function ConversionsTab({ ar, C }: { ar: boolean; C: any }) {
  // Unlike Salla/Zid, the Shopify backend returns a pre-computed
  // `totals` block (refunds + FX make a client-side sum unreliable) so
  // we trust the server numbers and just render the order list below.
  const { data, isLoading } = useShopifyConversions(15000);
      ...
      <Card style={{ padding: 24 }}>
        <div style={{ textAlign: "center", padding: 20 }}>
          ...
          <p style={{ margin: 0, fontSize: 12, color: C.t2, lineHeight: 1.7 }}>
            {ar
              ? "اضغط زرّ \"مزامنة\" أعلى الصفحة لجلب الطلبات الحاليّة من Shopify. يتمّ السحب التلقائي كلّ ساعة كذلك."
              : "Press the \"Sync\" button above to pull current orders from Shopify. Automatic sync also runs hourly."}
          </p>
        </div>
      </Card>
    );
  ...
  const fmtMoney = (n: number) => `${(Number(n) || 0).toFixed(2)} ${ar ? "ر.س" : "SAR"}`;
  // Shopify financial/fulfilment status vocabulary.
  const statusLabel = (s: string) => {
    const k = (s || "").toLowerCase();
    if (k === "paid") return ar ? "مدفوع" : "Paid";
    if (k === "pending") return ar ? "قيد الانتظار" : "Pending";
    if (k === "fulfilled") return ar ? "تم التنفيذ" : "Fulfilled";
    if (k === "shipped") return ar ? "مشحون" : "Shipped";
    if (k === "delivered") return ar ? "تم التسليم" : "Delivered";
    if (k === "refunded") return ar ? "مُسترَد" : "Refunded";
    if (k === "partially_refunded") return ar ? "مُسترَد جزئياً" : "Partially refunded";
    if (k === "cancelled" || k === "canceled") return ar ? "ملغي" : "Cancelled";
    return s;
  };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {/* Summary cards — server-computed totals. */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 12 }}>
        <SummaryCard ar={ar} C={C} label={ar ? "إجمالي الطلبات" : "Total orders"} value={(totals?.orders_count ?? orders.length).toLocaleString()} />
        <SummaryCard ar={ar} C={C} label={ar ? "عملاء فريدون" : "Unique buyers"} value={(totals?.unique_buyers ?? 0).toLocaleString()} />
        <SummaryCard ar={ar} C={C} label={ar ? "الإيراد الإجمالي" : "Gross revenue"} value={fmtMoney(totals?.gross_revenue ?? 0)} />
        <SummaryCard ar={ar} C={C} label={ar ? "المُسترَد" : "Refunded"} value={fmtMoney(totals?.refunded_sar ?? 0)} />
        <SummaryCard ar={ar} C={C} label={ar ? "الإيراد الصافي" : "Net revenue"} value={fmtMoney(totals?.net_revenue ?? 0)} />
      </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead style={{ background: C.bg2 || "#f5f5f5" }}>
              <tr>
                <th style={{ padding: "10px 12px", textAlign: "start", fontWeight: 600, color: C.t2, fontSize: 11 }}>{ar ? "رقم الطلب" : "Order #"}</th>
                ...
                <th style={{ padding: "10px 12px", textAlign: "start", fontWeight: 600, color: C.t2, fontSize: 11 }}>{ar ? "الحملة" : "Campaign"}</th>
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} style={{ borderTop: `1px solid ${C.bd}` }}>
                  <td style={{ padding: "10px 12px", color: C.txt, fontFamily: "monospace", fontSize: 12 }}>{o.order_number || o.shopify_order_id}</td>
...
function EventsTab({ ar, C }: { ar: boolean; C: any }) {
  // Polls every 5s so a webhook fired by Shopify shows up almost
  // immediately without the operator needing to refresh.
  const { data, isLoading } = useShopifyWebhookEvents(5000);
    ...
    return (
      <Card style={{ padding: 24 }}>
        <div style={{ textAlign: "center", padding: 20 }}>
          ...
          <p style={{ margin: 0, fontSize: 12, color: C.t2, lineHeight: 1.7 }}>
            {ar
              ? "ستظهر أحداث Shopify هنا فور وصولها (طلبات جديدة، عملاء، تنفيذ الشحن). افتح متجرك التجريبي وأنشئ طلباً للاختبار."
              : "Shopify events will appear here as they arrive. Place a test order in your store to verify."}
          </p>
        </div>
      </Card>
    );
            ...
            <div key={ev.id} style={{ padding: "10px 16px", borderBottom: `1px solid ${C.bd}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: C.t2 }}>
                  ...
                  {ev.shopify_event_id && <span style={{ marginInlineStart: 8, opacity: 0.7 }}>· {ev.shopify_event_id}</span>}
                </div>
                {failed && (
...
// ─── SettingsTab ──────────────────────────────────────────────────
//
// Order notification settings. Without these toggles the order-status
// notification job silently no-ops: every Shopify webhook gets
// processed but the customer never receives a WhatsApp message. The
// template picker is filtered server-side to approved utility/
// authentication templates only, preventing a marketing template from
// getting paired with an automatic (non-opt-in) order notification.
//
// Shopify MVP has NO cart-recovery block (unlike Salla/Zid).
...
function SettingsTab({ ar, C }: { ar: boolean; C: any }) {
  ...
  const [templates, setTemplates] = useState<ShopifyTemplate[]>([]);

  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/integrations/shopify/settings");
        setSettings(res.data.settings as ShopifySettings);
        setTemplates((res.data.templates ?? []) as ShopifyTemplate[]);
      } catch {
  ...
  const patch = async (changes: Partial<ShopifySettings>) => {
    if (!settings) return;
    const next = { ...settings, ...changes };
    setSettings(next); // optimistic
    setSaving(true);
    try {
      const res = await api.patch("/integrations/shopify/settings", changes);
      setSettings(res.data.settings as ShopifySettings);
    } catch (e: any) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* Order notifications block — the only Settings block for Shopify. */}
        ...
        {settings.order_notifications_enabled && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 12 }}>
            <TemplatePicker
              ...
              hint={ar ? "يُرسَل عند إنشاء الطلب (orders/create)" : "Fires on order creation (orders/create)"}
              expectedParams={[
            ...
            <TemplatePicker
              ...
              hint={ar ? "يُرسَل عند تنفيذ الشحن (fulfillments/create)" : "Fires on fulfillment (fulfillments/create)"}
              expectedParams={[
            ...
            <TemplatePicker
              ar={ar} C={C}
              label={ar ? "قالب: تمّ التسليم (اختياري)" : "Template: Delivered (optional)"}
              hint={ar
                ? "اختياري — شوبيفاي يطلق التأكيد عند الإنشاء والشحن عند التنفيذ؛ التسليم غير مستخدَم حالياً."
                : "Optional — Shopify fires confirmed on order create and shipped on fulfillment; delivered is unused for now."}
              expectedParams={[
                { num: 1, ar: "رقم الطلب", en: "Order number" },
              ]}
...
function ShopifyQuickGuide({ ar, C }: { ar: boolean; C: any }) {
  ...
  const steps = ar
    ? [
        ...
        },
        {
          ...
          body: "كل مرحلة (تأكيد/شحن) تحتاج عدد متغيّرات محدّد. القسم التالي يعرض المتغيّرات المطلوبة لكل مرحلة — انسخها بنفس الترتيب وإلّا Meta سترفض الإرسال.",
        },
        {
        ...
        {
          title: "4. اعمل طلب اختبار من متجر شوبيفاي",
    ...
    : [
        ...
        },
        {
          ...
          body: "Each stage (confirmed/shipped) needs a specific {{N}} count. The section below shows the required mapping per stage — copy it exactly or Meta will reject the send.",
        },
        {
        ...
        {
          title: "4. Place a test order in Shopify",
      ...
      {open && (
        <div style={{ padding: "0 18px 18px" }}>
          ...
          <div style={{
            ...
            {ar
              ? "💡 ملاحظة: الإشعارات تُرسَل فقط إلى أرقام موجودة في جهات الاتصال. لو زبون شوبيفاي جديد، يُنشأ contact تلقائياً عند ربط متجره — لا يلزم استيراد يدوي."
              : "💡 Tip: notifications only fire to numbers that exist in Contacts. New Shopify customers are auto-synced — no manual import needed."}
          </div>
        </div>
      )}
    </Card>
